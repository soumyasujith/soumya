...
shinyServer(function(input, output) {
  ...
  output$p1 <- renderPlot({
    if (input$graph_type == "histogram") {
      ...
      ggplot(df_country(), aes_string(x = input$continuous_variable)) +
        ...
        labs(y = "no.of people",title = paste("trend of",input$continuous_variable)) +  # labels
        facet_wrap(~prediction)    # facet by prediction
    }
    else {
      # Boxplot
      ggplot(df_country(), aes_string(y = input$continuous_variable)) +
        geom_boxplt(color = "chocolate", fill = "darkgoldenrod1") +  # boxplot geom
        coordflip() +  # flip coordinates
        labs(x = "no.of people",title = paste("trend of",input$continuous_variable)) +  # labels
        facet_wrap(~prediction)    # facet by prediction
    }
  ...
  output$p2 <- renderPlot({
    ...
    p <- ggplot(df_country(), aes_string(x = input$categorical_variable)) +
      labs(y = "no.of people", title = paste("trend of",input$categorical_variable)) +  # labels
      theme_light()
    ...
    if (input$is_stacked) {
      ...
      p + 
        ...
        facet_wrap(~prerdiction)   # facet by prediction
    }
  })
